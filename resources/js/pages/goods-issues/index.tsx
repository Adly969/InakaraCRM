...
import { index as indexIssuesRoute, create as createIssueRoute, show as showIssueRoute } from '@/routes/goods-issues';
import type { GoodsIssue } from '@/types';
import { Eye, Plus, ArrowUpFromLine, Search } from 'lucide-react';
...
export default function GoodsIssuesIndex({ goodsIssues, filters }: Props) {
    ...
    const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        router.get(indexIssuesRoute().url, { search }, { preserveState: true });
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                            <ArrowUpFromLine className="h-6 w-6 text-indigo-600 dark:text-indigo-400" />
                            <h1 className="text-2xl font-bold tracking-tight text-neutral-900 dark:text-neutral-50">
                                Goods Issues (GI)
                            </h1>
                        </div>
                        <p className="text-sm text-neutral-500 dark:text-neutral-400">
                            Manage outbound stock delivery orders, shipping logs, and sales allocations.
                        </p>
                    </div>
                    {can('create-goods-issues') && (
                        <Button asChild className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm">
                            <Link href={createIssueRoute().url}>
                ...
                <Card className="flex-1 overflow-hidden border-neutral-200/80 dark:border-neutral-800/80 bg-neutral-50/50 dark:bg-neutral-900/50 shadow-sm">
                    <CardContent className="p-0">
                                ...
                                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                                    {goodsIssues.data.length === 0 ? (
                                        ...
                                        goodsIssues.data.map((gi) => (
                                            <tr key={gi.id} className="hover:bg-neutral-100/30 dark:hover:bg-neutral-800/30 transition-colors">
                                                ...
                                                <td className="p-4 align-middle font-medium text-neutral-600 dark:text-neutral-400">
                                                    {gi.sales_order?.reference_no ?? 'Manual / Ad-hoc'}
                                                </td>
                                                ...
                                                <td className="p-4 align-middle text-right">
                                                    <Button variant="ghost" size="icon" asChild>
                                                        <Link href={showIssueRoute(gi.id).url}>
                        ...
                        {goodsIssues.links && goodsIssues.links.length > 3 && (
                            <div className="flex items-center justify-between border-t border-neutral-200 p-4 dark:border-neutral-800">
                                <div className="text-xs text-neutral-500 dark:text-neutral-400">
                                    Showing page {goodsIssues.current_page} of {goodsIssues.last_page} ({goodsIssues.total} total issues)
                                </div>
                                <div className="flex items-center gap-1">
                                    {goodsIssues.links.map((link, idx) => {

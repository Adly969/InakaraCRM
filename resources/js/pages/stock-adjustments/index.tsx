...
import { index as indexAdjustmentsRoute, create as createAdjustmentRoute, show as showAdjustmentRoute } from '@/routes/stock-adjustments';
import type { StockAdjustment } from '@/types';
import { Eye, Plus, Sliders, Search } from 'lucide-react';
...
export default function StockAdjustmentsIndex({ stockAdjustments, filters }: Props) {
    ...
    const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        router.get(indexAdjustmentsRoute().url, { search }, { preserveState: true });
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-1">
                        ...
                        <p className="text-sm text-neutral-500 dark:text-neutral-400">
                            Perform manual penyesuaian stok (+/-) and trace physical stock modifications.
                        </p>
                    </div>
                    {can('adjust-inventory') && (
                        <Button asChild className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm">
                            <Link href={createAdjustmentRoute().url}>
                ...
                <Card className="flex-1 overflow-hidden border-neutral-200/80 dark:border-neutral-800/80 bg-neutral-50/50 dark:bg-neutral-900/50 shadow-sm">
                    <CardContent className="p-0">
                            ...
                            <table className="w-full caption-bottom text-sm">
                                ...
                                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                                    {stockAdjustments.data.length === 0 ? (
                                        ...
                                        stockAdjustments.data.map((adj) => (
                                            <tr key={adj.id} className="hover:bg-neutral-100/30 dark:hover:bg-neutral-800/30 transition-colors">
                                                ...
                                                <td className="p-4 align-middle text-neutral-600 dark:text-neutral-400 font-medium truncate max-w-[200px]">
                                                    {adj.notes}
                                                </td>
                                                ...
                                                <td className="p-4 align-middle text-right">
                                                    <Button variant="ghost" size="icon" asChild>
                                                        <Link href={showAdjustmentRoute(adj.id).url}>
                        ...
                        {stockAdjustments.links && stockAdjustments.links.length > 3 && (
                            <div className="flex items-center justify-between border-t border-neutral-200 p-4 dark:border-neutral-800">
                                <div className="text-xs text-neutral-500 dark:text-neutral-400">
                                    Showing page {stockAdjustments.current_page} of {stockAdjustments.last_page} ({stockAdjustments.total} total adjustments)
                                </div>
